...
public final class KittenTTS: @unchecked Sendable {
    ...
    private func loadWeightsIfNeeded(modelPath: String? = nil) throws -> (Weights, [String: MLXArray]) {
        ...
        if let path = modelPath {
            ...
            throw NSError(domain: "KittenTTS", code: 2,
                          userInfo: [NSLocalizedDescriptionKey: "No model found. Call preload() or provide modelPath."])
        }

        let rawWeights = try loadArrays(url: modelDir.appendingPathComponent(Self.weightsFilename))
        let voices = try loadArrays(url: modelDir.appendingPathComponent("voices.safetensors"))
        let weights = Weights(rawWeights)
        cachedWeights = weights
        cachedVoices = voices
        return (weights, voices)
    }
    ...
    public func speak(
        ...
        return try await Device.withDefaultDevice(.gpu) {
            ...
            guard let voiceEmbeds = voices[voiceID] ?? voices["expr-voice-5-m"] else {
...
// MARK: - TextPreprocessor (number / currency / ordinal expansion)
...
enum TextPreprocessor {
    ...
    private static func expandCurrency(_ text: String) -> String {
        ...
        for match in re.matches(in: text, range: NSRange(location: 0, length: ns.length)).reversed() {
            let symbol     = match.range(at: 1).location != NSNotFound ? ns.substring(with: match.range(at: 1)) : ""
            var amountStr  = match.range(at: 2).location != NSNotFound ? ns.substring(with: match.range(at: 2)) : "0"
            let multiplier = match.range(at: 3).location != NSNotFound ? ns.substring(with: match.range(at: 3)) : ""
    ...
    private static func expandNumbers(_ text: String) -> String {
        ...
        if let re = try? NSRegularExpression(pattern: #"\b\d{1,3}(?:,\d{3})*\b|\b\d+\b"#) {
            let ns = t as NSString
            for match in re.matches(in: t, range: NSRange(location: 0, length: ns.length)).reversed() {
    ...
    private static func digitWord(_ d: String) -> String {
        ...
        case "9": return "nine"; default: return d
        }
    }
...
struct ModelLoader {
    ...
    public static func bundledVoiceIDs() -> [String] {
        guard let dir = bundledModelDir() else { return [] }
        let url = dir.appendingPathComponent("voices.safetensors")
        guard let voices = try? loadArrays(url: url) else { return [] }
        return voices.keys.sorted()
    }
}

// MARK: - Phonemizer (CEPhonemizer C++ bridge)

struct Phonemizer {
    static let pad = "$"
    static let symbols: [String] = {
        var s = ["$"]
        for char in ";:,.!?¡¿—…\"«»\"\" " { s.append(String(char)) }
        for char in "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz" { s.append(String(char)) }
        for scalar in "ɑɐɒæɓʙβɔɕçɗɖðʤəɘɚɛɜɝɞɟʄɡɠɢʛɦɧħɥʜɨɪʝɭɬɫɮʟɱɯɰŋɳɲɴøɵɸθœɶʘɹɺɾɻʀʁɽʂʃʈʧʉʊʋⱱʌɣɤʍχʎʏʑʐʒʔʡʕʢǀǁǂǃˈˌːˑʼʴʰʱʲʷˠˤ˞↓↑→↗↘'̩'ᵻ".unicodeScalars { s.append(String(scalar)) }
        return s
    }()
    static let dict: [String: Int] = Dictionary(
        symbols.enumerated().map { ($1, $0) },
    ...
    private static let handle: PhonemizerHandle? = {
        ...
        return phonemizer_create(rulesPath, listPath, "en-us")
    }()
    ...
    static func phonemize(_ text: String) throws -> [Int] {
        ...
        for scalar in ipa.unicodeScalars {
            if let idx = dict[String(scalar)] { tokens.append(idx) }
        }
        tokens.append(10); tokens.append(0)
        return tokens
    }
    ...
    private static func cePhonemize(_ text: String) throws -> String {
        guard let h = handle else {
            throw NSError(domain: "KittenTTS", code: 3,
                          userInfo: [NSLocalizedDescriptionKey: "CEPhonemizer failed to load (missing en_rules/en_list?)"])
        }
        guard let result = phonemizer_phonemize(h, text) else {
            throw NSError(domain: "KittenTTS", code: 4,
                          userInfo: [NSLocalizedDescriptionKey: "CEPhonemizer failed to phonemize text"])
        }
        let ipa = String(cString: result)
        phonemizer_free_string(result)

        // Preserve trailing punctuation (CEPhonemizer may strip it, upstream keeps it)
        var out = ipa
        let punctChars: Set<Character> = [".", "!", "?", ",", ";", ":", "…"]
        if let last = text.last, punctChars.contains(last), !out.hasSuffix(String(last)) {
            out += String(last)
        }
        return out
    }
...
// safetensors file. Most parameters exist as a (quantized, scale, zero_point)
// triple; a handful live as plain float tensors. `Weights` hides that plumbing
// and exposes name-based fetches in the ONNX layout. Layout conversion is done
// inside the NCL Conv helpers below so lookups stay verbatim with the Python
// reference in `kitten_mlx.py`.
...
struct Weights {
    ...
    func f32(_ key: String) -> MLXArray? {
        raw[key].map { $0.asType(.float32) }
    }

    /// Dequantize `kmodel.{base}.weight` using `weight_quantized/scale/zero_point`
    /// if present, otherwise returns the plain float `weight`.
    func dequant(_ base: String) -> MLXArray? {
        ...
        if let wq = raw["\(prefix).weight_quantized"],
           ...
           let z = raw["\(prefix).weight_zero_point"] {
            return (wq.asType(.float32) - z.asType(.float32)) * s.asType(.float32)
        }
        return raw["\(prefix).weight"]?.asType(.float32)
    }
    ...
    /// Dequantize an arbitrary `{base}_quantized/_scale/_zero_point` triple.
    func dequantRaw(_ base: String) -> MLXArray? {
        ...
        return raw[base]?.asType(.float32)
    }

    /// Dequantize a DynamicQuantizeLSTM matrix stored as
    /// `(num_dir, in, 4H)` int8 with per-direction scalar scale/zp. Returned
    /// shape `(num_dir, 4H, in)` — ONNX LSTM spec layout.
    func dequantLSTM(_ base: String) -> MLXArray? {
        guard let wq = raw["\(base)_quantized"],
              let s = raw["\(base)_scale"],
              let z = raw["\(base)_zero_point"] else { return nil }
        let sr = s.asType(.float32).reshaped([-1, 1, 1])
        let zr = z.asType(.float32).reshaped([-1, 1, 1])
        let dq = (wq.asType(.float32) - zr) * sr
        return dq.transposed(0, 2, 1)
    }
}

// MARK: - Activations / padding

private func leakyReLU(_ x: MLXArray, slope: Float) -> MLXArray {
    return MLX.where(x .> 0, x, x * slope)
}

/// Snake activation: `x + (1/alpha) * sin(alpha*x)^2`. `alpha` is (C,) and broadcasts on NCL.
...
/// Reflection pad on the LEFT by `n` samples along the last axis. Matches the
/// KittenTTS generator reflection pad between ups.1 and the MRF stack.
private func reflectionPadLeft(_ x: MLXArray, _ n: Int) -> MLXArray {
    guard n > 0 else { return x }
    let L = x.dim(-1)
    // Reflect indices: for n=1, pad element is x[..,1] (skip the boundary).
    let reflected = x[.ellipsis, 1...n]
    // Python slicing `x[..,1:n+1]` then reverse along last axis → left pad.
    let reversed = reflected[.ellipsis, .stride(by: -1)]
    _ = L
    return MLX.concatenated([reversed, x], axis: -1)
}

// MARK: - NCL Conv1d helpers
//
// ONNX stores Conv1d weights as (Cout, Cin, K) and MLX Conv1d operates on NLC
// tensors with weight (Cout, K, Cin). Since we're operating on many tensors
// whose layout is NCL (matching the Python port), and we need dilation support,
// we implement Conv1d/ConvTranspose1d directly on MLX arrays with numpy-style
// windowed matmuls.

/// Standard Conv1d on NCL tensor. Weight shape (Cout, Cin, K), bias (Cout,).
private func conv1dNCL(_ x: MLXArray, weight: MLXArray, bias: MLXArray?,
                       stride: Int = 1, padding: Int = 0, dilation: Int = 1) -> MLXArray {
    var xp = x
    if padding > 0 {
        xp = MLX.padded(xp, widths: [IntOrPair(0), IntOrPair(0), IntOrPair(padding)])
    }
    ...
    for i in 0..<K {
        // Slice xp along last axis with step=stride, offset=i*dilation, length=Lout.
        let startIdx = i * dilation
        let endIdx = startIdx + Lout * stride
        let xs = xp[0..., 0..., .stride(from: startIdx, to: endIdx, by: stride)]
        ...
        let contrib = MLX.matmul(xs.transposed(0, 2, 1), w_i.transposed(1, 0)).transposed(0, 2, 1)
        acc = acc.map { $0 + contrib } ?? contrib
    }
...
/// Depthwise Conv1d on NCL. Weight (C, 1, K) — one filter per channel.
private func conv1dDepthwiseNCL(_ x: MLXArray, weight: MLXArray, bias: MLXArray?,
                                stride: Int = 1, padding: Int = 0) -> MLXArray {
    var xp = x
    if padding > 0 {
        xp = MLX.padded(xp, widths: [IntOrPair(0), IntOrPair(0), IntOrPair(padding)])
    }
    ...
    for i in 0..<K {
        ...
        let xs = xp[0..., 0..., .stride(from: s, to: e, by: stride)]
...
/// Standard (group=1) ConvTranspose1d on NCL. Weight ONNX layout (Cin, Cout, K).
private func convTranspose1dNCL(_ x: MLXArray, weight: MLXArray, bias: MLXArray?,
                                stride: Int = 1, padding: Int = 0, outPadding: Int = 0) -> MLXArray {
    let B = x.dim(0); let _ = x.dim(1); let Lin = x.dim(2)
    let Cout = weight.dim(1); let K = weight.dim(2)
    let Lfull = (Lin - 1) * stride + K
    // Accumulate kernel-offset contributions into a zero-initialized buffer.
    var buf = MLX.zeros([B, Cout, Lfull])
    for k in 0..<K {
        // contrib: (B, Cout, Lin) = einsum('bcl,co->bol', x, w[:,:,k])
        let w_k = weight[0..., 0..., k]  // (Cin, Cout)
        let contrib = MLX.matmul(x.transposed(0, 2, 1), w_k).transposed(0, 2, 1)
        // Place contribution into buf at positions k, k+stride, k+2*stride, ...
        // Equivalent to: buf[:, :, k::stride][:, :, :Lin] += contrib
        // MLX has no scatter-add, so we build a one-hot mask and add.
        // For small K (≤20) and moderate Lfull, build-then-pad is fine:
        let before = k
        let after = Lfull - k - (Lin - 1) * stride - 1
        if after >= 0 {
            let spread = zeroInterleaveWithStride(contrib, stride: stride, zerosBefore: before, zerosAfter: after)
            buf = buf + spread
        } else {
            // Shouldn't happen given Lfull definition.
            let spread = zeroInterleaveWithStride(contrib, stride: stride, zerosBefore: before, zerosAfter: 0)
    ...
    if res.dim(-1) < Lout {
        ...
        res = MLX.padded(res, widths: [IntOrPair(0), IntOrPair(0), IntOrPair((0, pad))])
    }
...
private func convTranspose1dDepthwiseNCL(_ x: MLXArray, weight: MLXArray, bias: MLXArray?,
                                         stride: Int = 2, padding: Int = 1, outPadding: Int = 1) -> MLXArray {
    let B = x.dim(0); let C = x.dim(1); let Lin = x.dim(2)
    let K = weight.dim(-1)
    let Lfull = (Lin - 1) * stride + K
    var buf = MLX.zeros([B, C, Lfull])
    for k in 0..<K {
        let w_k = weight[0..., 0, k].reshaped([1, C, 1])  // (1, C, 1)
        let contrib = x * w_k  // (B, C, Lin)
        let before = k
        let after = Lfull - k - (Lin - 1) * stride - 1
        let spread = zeroInterleaveWithStride(contrib, stride: stride, zerosBefore: before, zerosAfter: max(0, after))
        let trimmed = spread[0..., 0..., 0..<Lfull]
        buf = buf + trimmed
    }
    let Lout = Lfull - 2 * padding + outPadding
...
/// Place `contrib` (B, C, Lin) at indices [zerosBefore, zerosBefore+stride, ...]
/// on a zero-filled track of length zerosBefore + (Lin-1)*stride + 1 + zerosAfter.
/// Implemented by interleaving zeros then left-/right-padding.
private func zeroInterleaveWithStride(_ x: MLXArray, stride: Int, zerosBefore: Int, zerosAfter: Int) -> MLXArray {
    var y = x
    if stride > 1 {
        // Interleave (stride-1) zero samples between each input sample along last axis.
        let B = y.dim(0); let C = y.dim(1); let Lin = y.dim(2)
        // Build (B, C, Lin, stride) where only index 0 holds x, rest are zero.
    ...
    if zerosBefore > 0 || zerosAfter > 0 {
        y = MLX.padded(y, widths: [IntOrPair(0), IntOrPair(0), IntOrPair((zerosBefore, zerosAfter))])
    }
    return y
}

// MARK: - LayerNorm / InstanceNorm

private func layerNormLast(_ x: MLXArray, weight: MLXArray?, bias: MLXArray?, eps: Float = 1e-5) -> MLXArray {
...
/// Channel-wise InstanceNorm1d on NCL (per-sample, per-channel over L axis). No affine.
private func instanceNorm1DNCL(_ x: MLXArray, eps: Float = 1e-5) -> MLXArray {
    let mean = x.mean(axis: -1, keepDims: true)
    let variance = x.variance(axis: -1, keepDims: true)
    return (x - mean) * MLX.rsqrt(variance + eps)
}

// MARK: - AdaIN / AdaLayerNorm

/// AdaIN1d. `style` (B, styleDim). `fc` projects to (B, 2C): split into gamma/beta.
/// Optional `normW`/`normB` apply affine after InstanceNorm (for blocks whose
/// PyTorch InstanceNorm was constructed with affine=True).
private func adaIN1D(_ x: MLXArray, style: MLXArray,
                     fcW: MLXArray, fcB: MLXArray,
                     normW: MLXArray?, normB: MLXArray?) -> MLXArray {
    // fcW stored as ONNX MatMul layout (styleDim, 2C); style (B, styleDim).
...
/// AdaLayerNorm on NLC: LayerNorm on last dim then (1+gamma)*x + beta with
/// gamma,beta from fc(style).
...
// MARK: - AdainResBlock1d (Predictor F0/N, decoder encode/decode)
...
private func loadAdaFC(_ w: Weights, base: String, which: String) -> AdaFC {
    let fcBase = "\(base).\(which).fc"
    let fcW = (w.dequant(fcBase) ?? w.f32("kmodel.\(fcBase).weight"))!
    let fcB = w.bias(fcBase)!
    let nw = w.raw["kmodel.\(base).\(which).norm.weight"]?.asType(.float32)
    let nb = w.raw["kmodel.\(base).\(which).norm.bias"]?.asType(.float32)
    return AdaFC(fcW: fcW, fcB: fcB, normW: nw, normB: nb)
}

/// AdainResBlock1d: norm1 → LeakyReLU → [pool upsample] → conv1 → norm2 →
/// LeakyReLU → conv2, with a (1+residual)/sqrt(2) shortcut. The shortcut can
/// take a distinct tensor (the upstream block's /sqrt(2)-normalized output).
private func adainResBlock1D(_ x: MLXArray, style: MLXArray, weights w: Weights,
                             base: String,
                             upsample: Bool = false,
                             divide: Bool = true,
    ...
    var h = adaIN1D(x, style: style, fcW: a1.fcW, fcB: a1.fcB, normW: a1.normW, normB: a1.normB)
    h = leakyReLU(h, slope: 0.2)

    if upsample {
        let poolW = w.f32("kmodel.\(base).pool.weight")!
        let poolB = w.f32("kmodel.\(base).pool.bias")
        h = convTranspose1dDepthwiseNCL(h, weight: poolW, bias: poolB, stride: 2, padding: 1, outPadding: 1)
    }

    let (c1W, c1B) = loadConv1dNCL(w, base: "\(base).conv1")
    let K1 = c1W.dim(-1)
    h = conv1dNCL(h, weight: c1W, bias: c1B, padding: (K1 - 1) / 2)

    h = adaIN1D(h, style: style, fcW: a2.fcW, fcB: a2.fcB, normW: a2.normW, normB: a2.normB)
    h = leakyReLU(h, slope: 0.2)
    let (c2W, c2B) = loadConv1dNCL(w, base: "\(base).conv2")
    let K2 = c2W.dim(-1)
    h = conv1dNCL(h, weight: c2W, bias: c2B, padding: (K2 - 1) / 2)

    let shortcutActive: Bool = hasConv1x1 ?? (
        upsample ||
        w.raw["kmodel.\(base).conv1x1.weight"] != nil ||
        w.raw["kmodel.\(base).conv1x1.weight_quantized"] != nil
    )
    let res: MLXArray
    if upsample {
...
private func loadConv1dNCL(_ w: Weights, base: String) -> (MLXArray, MLXArray?) {
    let weight = w.dequant(base)!
    let bias = w.bias(base)
    return (weight, bias)
}

// MARK: - HiFi-GAN Resblock (AdaIN + Snake, multi-receptive-field)

private func adainResBlockHiFiGAN(_ x: MLXArray, style: MLXArray, weights w: Weights,
                                  base: String, dilations: [Int] = [1, 3, 5]) -> MLXArray {
    var out = x
    for (k, d) in dilations.enumerated() {
        var h = out
        let a1 = loadAdaFC(w, base: base, which: "adain1.\(k)")
        h = adaIN1D(h, style: style, fcW: a1.fcW, fcB: a1.fcB, normW: a1.normW, normB: a1.normB)
        let alpha1 = w.f32("kmodel.\(base).alpha1.\(k)")!
        h = snake1D(h, alpha: alpha1)
        let (c1W, c1B) = loadConv1dNCL(w, base: "\(base).convs1.\(k)")
        let K = c1W.dim(-1)
        h = conv1dNCL(h, weight: c1W, bias: c1B, padding: d * (K - 1) / 2, dilation: d)

        let a2 = loadAdaFC(w, base: base, which: "adain2.\(k)")
        h = adaIN1D(h, style: style, fcW: a2.fcW, fcB: a2.fcB, normW: a2.normW, normB: a2.normB)
        let alpha2 = w.f32("kmodel.\(base).alpha2.\(k)")!
        h = snake1D(h, alpha: alpha2)
        let (c2W, c2B) = loadConv1dNCL(w, base: "\(base).convs2.\(k)")
        let K2 = c2W.dim(-1)
        h = conv1dNCL(h, weight: c2W, bias: c2B, padding: (K2 - 1) / 2)
...
// MARK: - ONNX LSTM forward (gate order iofc, bidirectional, per-direction scalar quant)

/// ONNX LSTM forward. `x` (seq, batch, in). `W` (num_dir, 4H, in). `R` (num_dir, 4H, H).
/// `B` (num_dir, 8H) = [Wb | Rb] (summed inside). Returns (seq, num_dir, batch, H).
private func onnxLSTMForward(_ x: MLXArray, W: MLXArray, R: MLXArray, B: MLXArray,
    ...
    func oneDir(_ d: Int, reverse: Bool) -> MLXArray {
        ...
        var outSteps: [MLXArray] = Array(repeating: MLX.zeros([batch, H]), count: seqLen)
        let range: StrideThrough<Int> = reverse
            ? stride(from: seqLen - 1, through: 0, by: -1)
            : stride(from: 0, through: seqLen - 1, by: 1)
        for t in range {
            let xt = x[t]  // (batch, in)
            let gates = MLX.matmul(xt, Wd.transposed(1, 0)) + MLX.matmul(h, Rd.transposed(1, 0)) + bias
    ...
    if !bidirectional {
        ...
        return yF.reshaped([seqLen, 1, batch, H])
    } else {
        let yF = oneDir(0, reverse: false)
        let yR = oneDir(1, reverse: true)
        return MLX.stacked([yF, yR], axis: 1)  // (seq, 2, batch, H)
    }
}

private func loadDQLSTM(_ w: Weights, W: String, R: String, B: String) -> (MLXArray, MLXArray, MLXArray) {
...
/// iSTFT decoder head. `convPostOut` is (1, 22, T); first 11ch = log-mag, last 11ch = phase.
/// Returns waveform (numSamples,) after trimming the tail by `tailTrim` samples
/// to match upstream kittentts (`audio[..., :-5000]`).
private func istftHead(_ convPostOut: MLXArray, weights w: Weights,
    ...
    let wReal = w.f32("kmodel.decoder.generator.stft.weight_backward_real")!
    let wImag = w.f32("kmodel.decoder.generator.stft.weight_backward_imag")!
    // ConvTranspose1d (Cin=11 → Cout=1, K=20, stride=5, pad=0).
    let audioReal = convTranspose1dNCL(real, weight: wReal, bias: nil, stride: 5, padding: 0)
    let audioImag = convTranspose1dNCL(imag, weight: wImag, bias: nil, stride: 5, padding: 0)
...
private func bertForward(_ inputIds: MLXArray, weights w: Weights) -> MLXArray {
    // Embeddings.
    ...
    // mapping_in (128 → 768). ONNX initializer: onnx::MatMul_5661 shape (128,768).
    let mIn = w.f32("onnx::MatMul_5661")!
    let mInB = w.bias("bert.encoder.embedding_hidden_mapping_in")!
    h = MLX.matmul(h, mIn) + mInB

    // 12 iterations of shared albert layer. ONNX MatMul initializer names resolved
    // from verify_out/kitten_exposed.onnx.
    ...
    for _ in 0..<12 {
        let B = h.dim(0); let Lh = h.dim(1); let D = 768; let nHead = 12; let dHead = 64
        let q = (MLX.matmul(h, qW) + qB).reshaped([B, Lh, nHead, dHead]).transposed(0, 2, 1, 3)
        let k = (MLX.matmul(h, kW) + kB).reshaped([B, Lh, nHead, dHead]).transposed(0, 2, 1, 3)
        let v = (MLX.matmul(h, vW) + vB).reshaped([B, Lh, nHead, dHead]).transposed(0, 2, 1, 3)
        let scores = MLX.matmul(q, k.transposed(0, 1, 3, 2)) / sqrt(Float(dHead))
        let attn = MLX.softmax(scores, axis: -1)
        let ctx = MLX.matmul(attn, v).transposed(0, 2, 1, 3).reshaped([B, Lh, D])
        let attOut = MLX.matmul(ctx, dW) + dB
        let hMid = layerNormLast(attOut + h, weight: attLnW, bias: attLnB, eps: 1e-12)
        let ffnH = MLX.matmul(hMid, ffnW) + ffnB
        let ffnAct = MLXNN.gelu(ffnH)
        let ffnRes = MLX.matmul(ffnAct, ffnOutW) + ffnOutB
        h = layerNormLast(ffnRes + hMid, weight: fullLnW, bias: fullLnB, eps: 1e-12)
    }
    return h
}

// MARK: - Predictor text encoder (LSTM / AdaLN / LSTM / AdaLN)

private func predictorTextEncoder(_ bertOutNLC: MLXArray, prosodyStyle: MLXArray, weights w: Weights) -> MLXArray {
    let B = bertOutNLC.dim(0); let L = bertOutNLC.dim(1)
    let sBcast = MLX.broadcast(prosodyStyle.reshaped([B, 1, -1]), to: [B, L, prosodyStyle.dim(-1)])

    // lstms.0: concat(bert, style) (1, L, 256) → bidir LSTM (hidden 64) → (1, L, 128)
    var x = MLX.concatenated([bertOutNLC, sBcast], axis: -1)
    var xT = x.transposed(1, 0, 2).asType(.float32)  // (L, 1, 256)
    var (W, R, Bv) = loadDQLSTM(w, W: "onnx::LSTM_5872", R: "onnx::LSTM_5873", B: "onnx::LSTM_5871")
    var y = onnxLSTMForward(xT, W: W, R: R, B: Bv, bidirectional: true)  // (L, 2, 1, 64)
    let y0 = y.transposed(2, 0, 1, 3).reshaped([B, L, 128])

    // lstms.1: AdaLN
    let fc1W = w.f32("kmodel.predictor.text_encoder.lstms.1.fc.weight")!
    let fc1B = w.f32("kmodel.predictor.text_encoder.lstms.1.fc.bias")!
    let y1 = adaLayerNorm(y0, style: prosodyStyle, fcW: fc1W, fcB: fc1B)

    // lstms.2: concat(y1, style) → bidir LSTM → (1, L, 128)
    x = MLX.concatenated([y1, sBcast], axis: -1)
    xT = x.transposed(1, 0, 2).asType(.float32)
    (W, R, Bv) = loadDQLSTM(w, W: "onnx::LSTM_5922", R: "onnx::LSTM_5923", B: "onnx::LSTM_5921")
    y = onnxLSTMForward(xT, W: W, R: R, B: Bv, bidirectional: true)
    let y2 = y.transposed(2, 0, 1, 3).reshaped([B, L, 128])

    // lstms.3: AdaLN
    let fc3W = w.f32("kmodel.predictor.text_encoder.lstms.3.fc.weight")!
    let fc3B = w.f32("kmodel.predictor.text_encoder.lstms.3.fc.bias")!
    return adaLayerNorm(y2, style: prosodyStyle, fcW: fc3W, fcB: fc3B)
}

// MARK: - Acoustic text encoder (embedding → 2×CNN → bidir LSTM → NCL)

private func acousticTextEncoder(_ inputIds: MLXArray, weights w: Weights) -> MLXArray {
    let wEmb = w.f32("kmodel.text_encoder.embedding.weight")!
    var xNLC = wEmb[inputIds[0]]  // (L, 128)
    xNLC = xNLC.reshaped([1, -1, 128])

    for i in 0..<2 {
        let (conv, bias) = loadConv1dNCL(w, base: "text_encoder.cnn.\(i).0")
        let K = conv.dim(-1)
        var xNCL = xNLC.transposed(0, 2, 1)
        xNCL = conv1dNCL(xNCL, weight: conv, bias: bias, padding: (K - 1) / 2)
        xNLC = xNCL.transposed(0, 2, 1)
        let gamma = w.f32("kmodel.text_encoder.cnn.\(i).1.gamma")!
        let beta = w.f32("kmodel.text_encoder.cnn.\(i).1.beta")!
    ...
    let (W, R, Bv) = loadDQLSTM(w, W: "onnx::LSTM_5652", R: "onnx::LSTM_5653", B: "onnx::LSTM_5651")
    let y = onnxLSTMForward(xT, W: W, R: R, B: Bv, bidirectional: true)  // (L, 2, 1, 64)
    let L = xT.dim(0)
    let yNLC = y.transposed(2, 0, 1, 3).reshaped([1, L, 128])
    return yNLC.transposed(0, 2, 1)  // (1, 128, L)
}

// MARK: - Length regulation

/// Build an alignment matrix (1, nPhonemes, nFrames): A[0,i,j]=1 iff phoneme i
/// spans frame j (block-diagonal one-hot). Used as a matmul to duration-expand
/// both prosody and acoustic features.
private func buildAlignment(durations: [Int32], nPhonemes: Int, nFrames: Int) -> MLXArray {
    var flat = [Float](repeating: 0.0, count: nPhonemes * nFrames)
    var t = 0
...
private func decoderPipeline(textFeaturesNCL: MLXArray, f0NCL: MLXArray, nNCL: MLXArray,
                             acousticStyle: MLXArray, weights w: Weights) -> MLXArray {
    // asr_res on text features (128 → 64, k=1).
    let (asrW, asrB) = loadConv1dNCL(w, base: "decoder.asr_res.0")
    let asr = conv1dNCL(textFeaturesNCL, weight: asrW, bias: asrB, padding: 0)
    // F0_conv / N_conv (stride 2, k=3, pad=1) downsample F0/N by 2.
    let (f0W, f0B) = loadConv1dNCL(w, base: "decoder.F0_conv")
    let f0Dn = conv1dNCL(f0NCL, weight: f0W, bias: f0B, stride: 2, padding: 1)
    let (nW, nB) = loadConv1dNCL(w, base: "decoder.N_conv")
    let nDn = conv1dNCL(nNCL, weight: nW, bias: nB, stride: 2, padding: 1)

    // encode input: concat(text (128), F0 (1), N (1)) = 130 channels.
    let encIn = MLX.concatenated([textFeaturesNCL, f0Dn, nDn], axis: 1)
    var x = adainResBlock1D(encIn, style: acousticStyle, weights: w, base: "decoder.encode",
                            upsample: false, divide: true, shortcutInput: encIn, hasConv1x1: true)

    // decode.0..3: concat(prev, asr_res (64), F0 (1), N (1)) = 322 channels. decode.3 upsamples.
    for i in 0..<4 {
        let xCat = MLX.concatenated([x, asr, f0Dn, nDn], axis: 1)
        let isUp = (i == 3)
        x = adainResBlock1D(xCat, style: acousticStyle, weights: w, base: "decoder.decode.\(i)",
                            upsample: isUp, divide: true, shortcutInput: xCat, hasConv1x1: true)
    }
    return x  // (1, 256 or 64 after decode.3, nFrames*2)
}

// MARK: - Generator pipeline (HiFi-GAN with iSTFT head)

private func generatorPipeline(_ decoderOutNCL: MLXArray, acousticStyle: MLXArray,
                               noiseRes0: MLXArray, noiseRes1: MLXArray,
    ...
    x = convTranspose1dNCL(x, weight: u0W, bias: u0B, stride: 10, padding: 5)
    x = x + noiseRes0

    // MRF: avg(resblocks.0, resblocks.1).
    let r0 = adainResBlockHiFiGAN(x, style: acousticStyle, weights: w, base: "decoder.generator.resblocks.0")
    let r1 = adainResBlockHiFiGAN(x, style: acousticStyle, weights: w, base: "decoder.generator.resblocks.1")
    x = (r0 + r1) / 2.0
    ...
    x = convTranspose1dNCL(x, weight: u1W, bias: u1B, stride: 6, padding: 3)

    // Reflection pad on the LEFT by 1 (KittenTTS quirk — NOT right).
    x = reflectionPadLeft(x, 1)
    x = x + noiseRes1

    let r2 = adainResBlockHiFiGAN(x, style: acousticStyle, weights: w, base: "decoder.generator.resblocks.2")
    let r3 = adainResBlockHiFiGAN(x, style: acousticStyle, weights: w, base: "decoder.generator.resblocks.3")
    x = (r2 + r3) / 2.0

    x = leakyReLU(x, slope: 0.1)
    let (cpW, cpB) = loadConv1dNCL(w, base: "decoder.generator.conv_post")
    x = conv1dNCL(x, weight: cpW, bias: cpB, padding: 3)  // (1, 22, T)
    return istftHead(x, weights: w)
}

// MARK: - Noise path (F0-based harmonic source)
//
// The real noise path: SineGen generates pitch-synchronised harmonic excitation
// from F0, a learned linear layer mixes the 9 harmonics down to one channel,
// a learned forward STFT (Conv1d real+imag) extracts magnitude+phase, and two
// strided noise_convs produce features that feed into the noise_res AdaIN
// ResBlocks. The outputs are added into the HiFi-GAN upsampling stages.

private func computeNoiseContribs(f0Proj: MLXArray, nFrames: Int, acousticStyle: MLXArray,
                                  weights w: Weights) -> (MLXArray, MLXArray) {
    ...
    // Nearest-neighbour upsample F0 by 300x: (1,1,T_frames) → (1,1,T_audio)
    let f0Expand = f0Proj.reshaped([1, 1, T_frames, 1])
    let f0Bcast = MLX.broadcast(f0Expand, to: [1, 1, T_frames, hopSize])
    ...
    let lLinW = (w.f32("onnx::MatMul_6116") ?? w.f32("onnx::MatMul_6388"))!  // (9, 1)
    let lLinB = w.bias("decoder.generator.m_source.l_linear")!                // (1,)
    let mixed = MLX.matmul(sinGen.transposed(0, 2, 1), lLinW) + lLinB         // (1, T, 1)
    let excitation = MLX.tanh(mixed.transposed(0, 2, 1))                      // (1, 1, T_audio)

    // ── 3. Forward STFT (learned Conv1d, not FFT) ──
    let stftR = w.f32("kmodel.decoder.generator.stft.weight_forward_real")!   // (11, 1, 20)
    let stftI = w.f32("kmodel.decoder.generator.stft.weight_forward_imag")!   // (11, 1, 20)
    let stftReal = conv1dNCL(excitation, weight: stftR, bias: nil, stride: 5, padding: 10)
    let stftImag = conv1dNCL(excitation, weight: stftI, bias: nil, stride: 5, padding: 10)
    let magSq = stftReal * stftReal + stftImag * stftImag
    let mag   = MLX.sqrt(magSq + Float(1e-9))                // (1, 11, T_stft)
    let phi   = atan2(stftImag, stftReal)                     // (1, 11, T_stft)
    let stftOut = MLX.concatenated([mag, phi], axis: 1)       // (1, 22, T_stft)

    // ── 4. noise_convs ──
    let (nc0W, nc0B) = loadConv1dNCL(w, base: "decoder.generator.noise_convs.0")
    let nc0 = conv1dNCL(stftOut, weight: nc0W, bias: nc0B, stride: 6, padding: 3)

    let (nc1W, nc1B) = loadConv1dNCL(w, base: "decoder.generator.noise_convs.1")
    let nc1 = conv1dNCL(stftOut, weight: nc1W, bias: nc1B)

    // ── 5. noise_res (AdaIN ResBlocks with Snake activation) ──
    let nr0 = adainResBlockHiFiGAN(nc0, style: acousticStyle, weights: w,
                                   base: "decoder.generator.noise_res.0")
    let nr1 = adainResBlockHiFiGAN(nc1, style: acousticStyle, weights: w,
                                   base: "decoder.generator.noise_res.1")
    return (nr0, nr1)
}

// MARK: - End-to-end forward

/// Tokens → waveform. `style256` first half is acoustic, second half prosodic.
func kittenForward(weights w: Weights, inputIds: MLXArray, style256: MLXArray,
                   speed: Float = 1.0) -> MLXArray {
    let B = inputIds.dim(0); let L = inputIds.dim(1)
    let acousticStyle = style256[0..., 0..<128].asType(.float32)   // (1, 128)
    let prosodicStyle = style256[0..., 128..<256].asType(.float32) // (1, 128)

    // BERT + bert_encoder (768 → 128).
    let bertOut = bertForward(inputIds, weights: w)  // (1, L, 768)
    let beW = w.f32("onnx::MatMul_5818")!  // (768, 128)
    let beB = w.bias("bert_encoder")!
    let prosodyIn = MLX.matmul(bertOut, beW) + beB  // (1, L, 128)

    // predictor.text_encoder (4-layer LSTM/AdaLN stack).
    let prosody = predictorTextEncoder(prosodyIn, prosodyStyle: prosodicStyle, weights: w)  // (1, L, 128)
    let sBcast = MLX.broadcast(prosodicStyle.reshaped([B, 1, -1]), to: [B, L, 128])
    let prosody256 = MLX.concatenated([prosody, sBcast], axis: -1)
    let prosodyNCL = prosody256.transposed(0, 2, 1)  // (1, 256, L)

    // predictor.lstm (shared duration/prosody LSTM).
    let lstmIn = prosodyNCL.transposed(2, 0, 1).asType(.float32)  // (L, 1, 256)
    let (dW, dR, dB) = loadDQLSTM(w, W: "onnx::LSTM_5971", R: "onnx::LSTM_5972", B: "onnx::LSTM_5970")
    let dy = onnxLSTMForward(lstmIn, W: dW, R: dR, B: dB, bidirectional: true)
    let lstmOut = dy.transposed(2, 0, 1, 3).reshaped([1, L, 128])

    // duration_proj: (128 → 50) per frame, sigmoid, sum → duration-per-phoneme.
    let dpW = w.f32("onnx::MatMul_5973")!  // (128, 50)
    let dpB = w.bias("predictor.duration_proj.linear_layer")!
    let durLogits = MLX.matmul(lstmOut, dpW) + dpB  // (1, L, 50)
    let durSig = MLX.sigmoid(durLogits)
    let durSum = durSig.sum(axis: -1)  // (1, L)
    let durScaled = durSum[0] / MLXArray(speed)
    let durs = MLX.maximum(MLXArray([1]), MLX.round(durScaled).asType(.int32))
    let durArr: [Int32] = durs.asArray(Int32.self)
    let nFrames = Int(durArr.reduce(0, +))

    // Alignment matrix + length regulation.
    let align = buildAlignment(durations: durArr, nPhonemes: L, nFrames: nFrames)
    let prosodyLRNCL = MLX.matmul(prosodyNCL, align)  // (1, 256, nFrames)

    // shared LSTM → (1, 128, nFrames).
    let sharedIn = prosodyLRNCL.transposed(2, 0, 1).asType(.float32)
    let (sW, sR, sB) = loadDQLSTM(w, W: "onnx::LSTM_6020", R: "onnx::LSTM_6021", B: "onnx::LSTM_6019")
    let sy = onnxLSTMForward(sharedIn, W: sW, R: sR, B: sB, bidirectional: true)
    let fnLSTMNLC = sy.transposed(2, 0, 1, 3).reshaped([1, nFrames, 128])
    let fnInNCL = fnLSTMNLC.transposed(0, 2, 1)  // (1, 128, nFrames)

    // F0 stack.
    var f0 = adainResBlock1D(fnInNCL, style: prosodicStyle, weights: w, base: "predictor.F0.0",
                             upsample: false, divide: true, shortcutInput: fnInNCL)
    f0 = adainResBlock1D(f0, style: prosodicStyle, weights: w, base: "predictor.F0.1",
                         upsample: true, divide: true, shortcutInput: f0)
    f0 = adainResBlock1D(f0, style: prosodicStyle, weights: w, base: "predictor.F0.2",
                         upsample: false, divide: true, shortcutInput: f0)
    let (f0pW, f0pB) = loadConv1dNCL(w, base: "predictor.F0_proj")
    let f0Proj = conv1dNCL(f0, weight: f0pW, bias: f0pB)  // (1, 1, nFrames*2)

    // N stack.
    var n = adainResBlock1D(fnInNCL, style: prosodicStyle, weights: w, base: "predictor.N.0",
                            upsample: false, divide: true, shortcutInput: fnInNCL)
    n = adainResBlock1D(n, style: prosodicStyle, weights: w, base: "predictor.N.1",
                        upsample: true, divide: true, shortcutInput: n)
    n = adainResBlock1D(n, style: prosodicStyle, weights: w, base: "predictor.N.2",
                        upsample: false, divide: true, shortcutInput: n)
    ...
    let textFeaturesNCL = acousticTextEncoder(inputIds, weights: w)  // (1, 128, L)
    let textLRNCL = MLX.matmul(textFeaturesNCL, align)  // (1, 128, nFrames)

    // Decoder.
    let decOut = decoderPipeline(textFeaturesNCL: textLRNCL, f0NCL: f0Proj, nNCL: nProj,
                                 acousticStyle: acousticStyle, weights: w)

    // Noise path: F0 → SineGen → STFT → noise_convs → noise_res.
    let (nr0, nr1) = computeNoiseContribs(f0Proj: f0Proj, nFrames: nFrames,
                                          acousticStyle: acousticStyle, weights: w)

    return generatorPipeline(decOut, acousticStyle: acousticStyle,
                             noiseRes0: nr0, noiseRes1: nr1, weights: w)
}
