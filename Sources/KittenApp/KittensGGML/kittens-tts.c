...
static int gguf_bool(const struct gguf_context * g, const char * key, int dflt) {
    ...
    return gguf_get_val_bool(g, idx) ? 1 : 0;
}
...
static gt kt_reflection_pad_left(struct ggml_context * c, gt x, int n) {
    if (n <= 0) return x;
    if (n != 1) KT_DIE("kt_reflection_pad_left only supports n=1 (got %d)", n);
    const int64_t C = x->ne[0];
    // View x[:, 1:2] — slice along ne[1] (the L axis).
    // ggml_view_2d: (ctx, t, ne0, ne1, nb1, offset).
    gt slice = ggml_view_2d(c, x, C, 1, x->nb[1], (size_t) 1 * x->nb[1]);
    slice = ggml_cont(c, slice);             // (C, 1)
    return ggml_concat(c, slice, x, /*dim=*/1);   // (C, L+1)
}
...
kt_ctx * kt_create(const char * gguf_path, const char * backend_name) {
    ...
    kt_ctx * ctx = (kt_ctx *) calloc(1, sizeof(kt_ctx));
    if (!ctx) { ggml_backend_free(b); return NULL; }
    ctx->backend = b;
                       ...
                       const float * style256, float speed)
{
    ...
    // All bufs declared up front + initialised to NULL so the `fail:` cleanup
    // can free unconditionally regardless of where we jump from.
    ...
    int F = 0;
    for (int i = 0; i < L; i++) {
        // dur layout: NLC ne=(50, L) so data[t*50 + j].  But actually run_textstage
        // writes ggml NLC linear which is data[l*50+j].  For NLC ne=(50, L), linear
        // index of (j, l) is j + l*50. So dur[l*50 + j] is sigmoid output for
        // token l at logit index j.
        float sum = 0;
    ...
    if (!prosody_lr || !text_lr) goto fail;
    {
        // prosody NLC ne=(256, L) → data[l*256 + c]. Same for text NLC (128, L).
        // Output prosody_lr ne=(256, F) → data[t*256 + c]. Expand each token l
        // into durs[l] consecutive frames.
        int t = 0;
        for (int l = 0; l < L; l++) {
            const int d = durs[l];
            for (int k = 0; k < d; k++, t++) {
                memcpy(prosody_lr + t * 256, prosody + l * 256, sizeof(float) * 256);
                memcpy(text_lr    + t * 128, text    + l * 128, sizeof(float) * 128);
            }
        }
    }
    ...
    int T_audio = 0;
    float * audio = kt_run_fullgen(m, F, dec_out, f0p, style256, &T_audio);
    ...
    int n = T_audio;
    const int tail_drop = 3 * 600;       // 3 frames × 600 samples/frame
...
fail:
    ...
    snprintf(ctx->err, sizeof(ctx->err), "kt_synthesize: allocation or stage failure");
    return out;
}
...
static ggml_backend_t kt_backend_init(const char * name) {
    if (!name || !strcmp(name, "cpu")) return ggml_backend_cpu_init();
#ifdef KT_HAVE_METAL
    if (!strcmp(name, "metal")) return ggml_backend_metal_init();
#endif
